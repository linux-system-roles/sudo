...
description:
    - "This module is designed to collect information from C(/etc/sudoers)."
    - "The #include (files) and #includedir (directories) will be dynamically calculated and all included files will be parsed."
...
def get_includes(path):
    ...
    for sline in sudoers_file.strip().split("\n"):
        line = sline.replace("\n", "").replace("\t", "    ")
    ...
    if include_dir:
        ...
        includes["include_files"] += [
            join(include_dir, filename)
            for filename in os.listdir(include_dir)
            if isfile(join(include_dir, filename))
        ]
...
def get_config_lines(path, params):
    ...
    for sline in all_lines.strip().split("\n"):
        ...
        if params["output_raw_configs"]:
            ...
            if comment_re.search(line) is None and line != "" and line is not None:
        ...
        if params["output_parsed_configs"]:
            ...
            if defaults_re.search(line):
                ...
                # build secure path dict and append to defaults list
                    ...
                    secure_paths = (
                        defaults_sec_path_re.search(defaults_config_line)
                        .group(5)
                        .split(":")
                    )
            ...
            if cmnd_alias_re.search(line):
                if cmnd_alias_re.search(line).group(5) == ":":
                    ...
                    # Process each alias
                    ca_multi_re = re.compile(
                        r"(^Cmnd_Alias)*\s*(\S+)+\s*\={1}\s*((\S+,{1}\s*)+\S+|\S+).*$"
                    )
            ...
            if host_alias_re.search(line):
                if host_alias_re.search(line).group(5) == ":":
                    ...
                    # Process each alias
                    ha_multi_re = re.compile(
                        r"(^Host_Alias)*\s*(\S+)+\s*\={1}\s*((\S+,{1}\s*)+\S+|\S+).*$"
                    )
            ...
            if runas_alias_re.search(line):
                if runas_alias_re.search(line).group(5) == ":":
                    ...
                    # Process each alias
                    ra_multi_re = re.compile(
                        r"""
                        (^Runas_Alias)*            # Optionally match 'Runas_Alias' at the start of the line
                        \s*                       # Optional whitespace
                        (\S+)+                    # One or more non-whitespace characters (alias name)
                        \s*                       # Optional whitespace
                        \={1}                     # Exactly one equals sign
                        \s*                       # Optional whitespace
                        (                        # Start of the group for the right-hand side of the equals sign
                            (\S+,{1}\s*)+        # One or more sequences of non-whitespace characters followed by a comma and optional whitespace
                            \S+                  # Followed by one or more non-whitespace characters
                            |\S+                 # Or just a single non-whitespace string
                        )                        # End of the group
                        .*$                       # Match the rest of the line
                        """,
                        re.VERBOSE,
                    )
                    for ra in runas_multi_line_aliases:
            ...
            if user_alias_re.search(line):
                if user_alias_re.search(line).group(5) == ":":
                    ...
                    # Process each alias
                    ua_multi_re = re.compile(
                        r"""
                        (^User_Alias)*            # Optionally match 'User_Alias' at the beginning
                        \s*                       # Optional whitespace
                        (\S+)+                    # One or more non-whitespace characters (alias name)
                        \s*                       # Optional whitespace
                        \={1}                     # Exactly one equals sign
                        \s*                       # Optional whitespace
                        ((\S+,{1}\s*)+\S+|\S+)     # One or more non-whitespace characters separated by commas, or a single non-whitespace string
                        .*$                       # Match the rest of the line
                        """,
                        re.VERBOSE,
                    )
            ...
            if (
                not user_alias_re.search(line)
                and not runas_alias_re.search(line)
                and not host_alias_re.search(line)
                and not cmnd_alias_re.search(line)
                and not include_re.search(line)
                and not comment_re.search(line)
                and not defaults_re.search(line)
                and line != ""
                and line is not None
            ):
                user_spec = get_user_specs(line)
    ...
    if params["output_parsed_configs"]:
        ...
        sudoer_aliases = {
            ...
            "runas_alias": runas_aliases,
            "cmnd_alias": command_aliases,
            "host_alias": host_aliases,
        }
...
def get_user_specs(line):
    user_spec = dict()
    user_spec_re = re.compile(
        r"""
        (^\S+,{1}\s*\S+|^\S+)                 # First part of the string (non-whitespace characters)
        \s*                                   # Optional whitespace
        (\S+,{1}\s*|\S+){1}                   # Second part (comma-separated or not)
        \s*={1}\s*                            # Equals sign, with optional surrounding whitespace
        (\({1}(.*)\){1})*                     # Optional parentheses around content
        \s*                                   # Optional whitespace
        (ROLE\s*=\s*(\S+)|TYPE\s*=\s*(\S+))*  # Optional ROLE or TYPE with values
        \s*                                   # Optional whitespace
        (ROLE\s*=\s*(\S+)|TYPE\s*=\s*(\S+))*  # Optional ROLE or TYPE repeated
        \s*                                   # Optional whitespace
        (PRIVS\s*=\s*(\S+)|LIMITPRIVS\s*=\s*(\S+))*  # Optional PRIVS or LIMITPRIVS with values
        \s*                                   # Optional whitespace
        (PRIVS\s*=\s*(\S+)|LIMITPRIVS\s*=\s*(\S+))*  # Optional PRIVS or LIMITPRIVS repeated
        \s*                                   # Optional whitespace
        (\S+:{1})*                            # Optional section with colon
        \s*(.*$)                              # Capture the rest of the line
        """,
        re.VERBOSE,
    )
    default_override_re = re.compile(
        r"""
        (Defaults)            # Match the word 'Defaults'
        {1}                   # Ensure it's matched exactly once
        ([@:!>]){1}           # Match one of the special characters: @, :, !, or >
        ((\s*\S+,{1})+        # Match one or more non-whitespace characters followed by a comma
        \s*\S+|\S+)           # Optionally match more non-whitespace characters, or a single non-whitespace string
        \s*                   # Optional whitespace
        (.*$)                 # Match the rest of the line
        """,
        re.VERBOSE,
        ...
        if spec_fields.group(11) or spec_fields.group(14):
            ...
            if spec_fields.group(11):
                if limitprivs_re.search(spec_fields.group(11)):
                    if (
                        limitprivs_re.search(spec_fields.group(11)).group(1)
                        == "LIMITPRIVS"
                    ):
            ...
            if spec_fields.group(14):
                if limitprivs_re.search(spec_fields.group(14)):
                    if (
                        limitprivs_re.search(spec_fields.group(14)).group(1)
                        == "LIMITPRIVS"
                    ):
...
def get_sudoers_configs(path, params):
    sudoers = dict()
    include_files = list()

    default_sudoers = path

    # Get parsed values from default sudoers file
    sudoers["sudoers_files"] = list()
    default = get_config_lines(path, params)
    if default:
    ...
    for file in include_files:
        include_file = get_config_lines(file, params)
        if include_file:
...
def main():
    ...
    sudoers = get_sudoers_configs(default_sudoers, params)
